...
const TagPage: React.FC = () => {
  ...
  const { tags, posts, loading } = useData();
  ...
  // Show loading state
  if (loading.posts || loading.tags) {
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="text-center">
          <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-primary-600 mx-auto mb-4"></div>
          <p className="text-gray-600 dark:text-gray-400">Loading tag...</p>
        </div>
      </div>
    );
  }
  ...
  if (!tag) {
    return (
      <div className="text-center py-20">
        <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Tag not found.</h1>
    ...
    >
      ...
      {currentPosts.length > 0 ? (
        ...
        <div className="text-center py-16 border-t border-gray-200 dark:border-zinc-700">
            <p className="text-gray-500 dark:text-zinc-400 text-lg">There are no stories with this tag yet.</p>
        </div>
      )}
    </motion.div>
  );
